...
<!-- pred <- predict(gbm.fit.final, n.trees = gbm.fit.final$n.trees, select_patients_df) -->

<!-- # results -->
<!-- caret::RMSE(pred, select_patients_df$PANSS_Total) -->
...
<!-- features_test <- vtreat::prepare(treatplan, select_patients_df, varRestriction = new_vars) %>% as.matrix() -->
<!-- response_test <- select_patients_df$PANSS_Total -->
...
<!-- # predict values for test data -->
<!-- pred <- predict(xgb.fit.final, select_patients_df) -->
...
#select_patients_df = subset(select_patients_df, select = c(PatientID, TxGroup, VisitDay, Study,PANSS_Total))
```
...
<!-- # Calculate test MSE -->
<!-- mean((select_patients_df$PANSS_Total - predict(linear.mod, select_patients_df))^2) -->
<!-- ``` -->
...
<!-- test.mat = model.matrix(PANSS_Total ~ .-PatientID, data = select_patients_df) -->
...
<!-- mean((ridge.pred - select_patients_df$PANSS_Total)^2) -->
<!-- ``` -->
...
<!-- mean((lasso.pred-select_patients_df$PANSS_Total)^2) -->
...
<!-- plot(ridge.pred,select_patients_df$PANSS_Total,xlim=c(20,100), ylim=c(20,100)) -->

<!-- # minimum Lasso MSE -->
<!-- min(lasso.cv.out$cvm) -->
<!-- plot(lasso.pred,select_patients_df$PANSS_Total,xlim=c(20,100), ylim=c(20,100)) -->
<!-- ``` -->
...
```{r test-mse}
test_predict = predict(tuned_mars, select_patients_df)
mean((select_patients_df$PANSS_Total - test_predict)^2)
plot(as.vector(test_predict), select_patients_df$PANSS_Total,xlim=c(30,100), ylim=c(30,100)) 
abline(0,1) 
