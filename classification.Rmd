...
tune.out <- tune(svm, LeadStatus~., kernel = "linear", data = combined.train[,c("TxGroup","VisitDay","PANSS_Total","LeadStatus")], ranges = list(cost = 10^(-2:1)), probability=TRUE)
...
```
We get a marginal improvement in logloss. Note that I only tuned for maximum of 1 minute here. Let's predict the Kaggle test set:
```{r test}
...
getPredict(best_model,test,"Predictions/test.gbm.csv")
```
