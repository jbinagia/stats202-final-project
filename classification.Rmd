...
```{r svm-radial-tune}
# find optimal cost of misclassification
tune.out <- tune(svm, LeadStatus~., kernel = "radial", data = combined.train[,c("TxGroup","VisitDay","PANSS_Total","LeadStatus")], ranges = list(cost = 10^(-2:1),gamma=10^(-2:1)), probability=TRUE)
# extract the best model
summary(tune.out)
(bestmod <- tune.out$best.model)
```
