...
- then gbm and rf which also sucked. BUT gbm and SVM had lower development set loglosses, so we'll submit anyway.
...
```{r lasso-performance}
#get dev set data
x_test <- model.matrix(LeadStatus~., combined.dev.all)
...
#kaggle test
x_test <- model.matrix(test.all$LeadStatus~., test.all)
lasso_prob <- predict(cv.out, newx = x_test, s=lambda_1se, type="response")
